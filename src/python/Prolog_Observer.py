...
class NimObserver(PrologObserver):
    def __init__(self, params, game_info):
        """init an empty observation tensor"""
        if params:
            raise ValueError(f"Observation parameters not supported; passed {params}")

        shape = (1 + game_info.num_players, 1, game_info.max_game_length)
        self.tensor = np.zeros(np.prod(shape), np.float32)
        self.dict = {"observation": np.reshape(self.tensor, shape)}
...
class Connect4Observer(PrologObserver):
    def __init__(self, params, game_info):
        ...
        self._NUM_PLAYERS = game_info.num_players
        self._NUM_COLS = int(game_info.max_game_length / game_info.num_distinct_actions)
        self._NUM_ROWS = game_info.num_distinct_actions
