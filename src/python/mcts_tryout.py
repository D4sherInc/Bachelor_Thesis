""" Das ist eine von https://github.com/deepmind/open_spiel/blob/master/open_spiel/python/examples/mcts.py,
angepasst als Test für die Implementation der Prolog-Anbindung. FLAGS wurden neu gesetzt und die Spieledefinition von
pyspiel.load_game wird ersetzt durch die Spieledefinition von Prolog_Game"""
...
flags.DEFINE_integer("num_games", 200, "How many games to play.")
...
def main(argv):
    logging.basicConfig(filename="mcts_tic_tac_toe_log.txt", filemode="a",
                        datefmt="%H:%M:%S",
    ...
    overall_wins = [0, 0]
    game_num = 0
    logger.info("%s: %s against %s", FLAGS.game, FLAGS.player1, FLAGS.player2)
    try:
        for _ in range(5):
            for game_num in range(FLAGS.num_games):
                ...
                for i, v in enumerate(returns):
                    ...
                    if v > 0:
                        overall_wins[i] += 1
            logger.info("%s %s", overall_wins[0], overall_wins[1])
